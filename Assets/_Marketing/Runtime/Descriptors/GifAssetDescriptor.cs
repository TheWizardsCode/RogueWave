...
namespace WizardsCode.Marketing
{
    ...
    public class GifAssetDescriptor : AssetDescriptor
    {
        ...
        public string GifPath
        {
            get
            {
                return $"Assets/Recordings/{AssetName}/{SceneName}/GIF/";
            }
        }
        ...
        public string SpritesheetPath
        {
            get
            {
                string path = $"Assets/Recordings/{AssetName}/{SceneName}/Spritesheet/";
        ...
        public string GifFilename
        {
            get
            {
                ...
                return $"{SceneName}_{AssetName}_{date}_{gifCount.ToString("D4")}";
            }
        }
        ...
        public override IEnumerator GenerateRequiredAssets(Action callback = null)
        {
            ...
            {
                SaveAsSpritesheet(128, 128);
            }
            if (m_SaveAsSmallSpriteSheet)
            {
                SaveAsSpritesheet(64, 64);
            }
        ...
        void SaveAsSpritesheet(int imageWidth, int imageHeight)
        {
            List<Texture2D> frames = LoadGif($"{GifPath}{GifFilename}.gif");
            Texture2D spritesheet = CreateSpritesheet(frames, imageWidth, imageHeight);
        ...
        Texture2D CreateSpritesheet(List<Texture2D> frames, int imageWidth, int imageHeight)
        {
            // Calculate the number of columns and rows such that there is the minimum number of each
            int columns = Mathf.CeilToInt(Mathf.Sqrt(frames.Count));
            int rows = Mathf.CeilToInt(frames.Count / (float)columns);

            Texture2D spritesheet = new Texture2D(imageWidth * columns, imageHeight * rows);

            for (int i = 0; i < frames.Count; i++)
            {
                int row = i / columns;
                int column = i % columns;
                Texture2D resizedFrame = ResizeTexture(frames[i], imageWidth, imageHeight);
                Color[] pixels = resizedFrame.GetPixels();
                spritesheet.SetPixels(column * imageWidth, (rows - row - 1) * imageHeight, imageWidth, imageHeight, pixels);
            }
...
#region MGGif
    // From https://github.com/gwaredd/mgGif
    // MIT Licensed - https://github.com/gwaredd/mgGif/blob/main/LICENSE
