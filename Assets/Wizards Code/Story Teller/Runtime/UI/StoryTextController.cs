...
namespace WizardsCode.StoryTeller
{
    public class StoryTextController : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("The GUI component that will display the speakers name.")]
        ...
        [SerializeField, Tooltip("Whether or not sounds should be played.")]
        bool m_PlaySpeakingSounds = false;
        private Coroutine revealCo;
        [SerializeField, Tooltip("An array of sounds that will be played while narration or speech is occuring"), ShowIf("m_PlaySpeakingSounds")]
        AudioClip[] m_SpeechSounds;
        [SerializeField, Tooltip("An array of sounds that will be played when punctuation is detected."), ShowIf("m_PlaySpeakingSounds")]
        AudioClip[] m_PunctuationSounds;
        [SerializeField, Tooltip("The audio source for the speech sounds."), ShowIf("m_PlaySpeakingSounds")]
        AudioSource m_AudioSource_Speech;
        [SerializeField, Tooltip("The audio source for the punctuation sounds."), ShowIf("m_PlaySpeakingSounds")]
        AudioSource m_AudioSourcePunctuation;

        [SerializeField, Tooltip("RUNTIME ONLY: The delay between characters being printed. If set to 0 all characters will be printed at once." +
        ...
        internal void AddText(IActorController speaker, string text)
        {
        ...
        public void SetText(IActorController speaker, string text)
        {
        ...
        public void SetText(IActorController speaker, string text, bool bPlaySpeakingSounds)
        {
            if (m_ClearOnNewText)
            {
                ClearText();
            ...
            if (m_ActiveSpeaker != null)
            {
                ...
                m_SpeakersName.transform.gameObject.SetActive(true);
            }
            else
            {
                // FIXME: shouldn't be navigating the tree like this, make the speakers name element the root object and discover the text object beneath
                m_SpeakersName.transform.gameObject.SetActive(false);
            }
        ...
        public void SetText(IActorController speaker, string text, bool bPlaySpeakingSounds, float secondsBetweenPrintingChars)
        {
