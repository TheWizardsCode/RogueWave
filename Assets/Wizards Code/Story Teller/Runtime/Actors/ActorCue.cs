...
namespace WizardsCode.StoryTeller
{
    ...
    public class ActorCue : ScriptableObject
    {
        ...
        protected IActorController m_Actor;
        ...
        public virtual IEnumerator Prompt(IActorController actor)
        {
            m_Actor = actor;
        ...
        /// </summary>
        /// <returns>An optional coroutine that shouold be started by the calling MonoBehaviour</returns>
        public virtual IEnumerator Revert(IActorController actor)
        {
        ...
        protected virtual void ProcessMove()
        {
            ...
            if (!string.IsNullOrWhiteSpace(markName))
            {
                ...
                {
                    Debug.LogWarning(m_Actor + "  has a mark set, but the mark doesn't exist in the scene. The name set is " + markName);
                }
            }
        }
        ...
        protected virtual void ProcessAudio()
        {
            if (audioClip != null)
            {
                AudioSource source = ((Component)m_Actor).GetComponent<AudioSource>();
