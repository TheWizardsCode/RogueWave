...
You will need to import [NeoFPS](https://bit.ly/NeoFPS). ***Note*** that by default Neo FPS will overwrite some files that are provided by this repo. After importing Neo FPS you need revert any changes made by the import. The easiest way to do this is to import Neo FPS and then run the command `git restore .` in the root of the project directory.
